...
| Generators | MCP `abap_generators-list_generators`/`get_schema` | ✅ / ◐ | `list_generators` ✅. `get_schema` **requires `packageName`** ("packageName is missing or empty" without it) — `get_generator_schema` now defaults `$TMP` (fixed). ⚠ a4h's generators (`uiservice`/`webapiservice`) are **object-referencing** (build a service *from* an object) → also need a `referencedObjectType` (e.g. `TABL`) + referenced object, else "referencedObjectType is not valid. Expected one of: TABL". Passthrough of those params is a **follow-up** (needs a live spike of the exact arg shape). |
